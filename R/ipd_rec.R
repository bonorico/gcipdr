...
Generate.with.Complete.correlation <- function(H, n, correlation.matrix, moments, johnson.parameters, stochastic.integration, x.mode,
                 ...
                 marg.model=c("gamma", "johnson"), SI_k = 8000){  
...
nortasd <- sqrt( Ind*( mx*(1-mx) ) ) + (1-Ind)*sdx
    ...
    res <- norta.method( H, n, correlation.matrix, mx, nortasd, stochastic.integration, norm.perc, marginals, variable.names, corrtype, SI_k)
...
if (any(norm3 > 1, na.rm = T))
  ...
  mex0 <- warning("some correlation entries differ from reference on average")
   ...
   if (split)
    ...
    if (tabulate){
ltRxdat <- data.frame( ipd = correlation.matrix[ltRx], mc.mean = lowtri.mc.average )
...
# similar data object, arg method = 1 to 8
 ...
 Simulate.data.given.IPD <- function( data, H = NULL, method, fill.missing = F,  # TODO(me) : u may want to set fill.missing = TRUE later to ensure maximal generality
                ...
                tabulate.similar.data =  FALSE, print.message = TRUE, set.corr.matr2null = FALSE, SI_k = 8000){
   ...
   if (set.corr.matr2null)
     Rx <- NULL
       ...
       data.simulation <- DataRebuild( H, n, Rx, moms, x.mode, jsp, stochastic.integration, data.rearrange, corrtype, marg.model,
                                  variable.names, SBjohn.correction, compute.eec, checkdata, tabulate.similar.data, SI_k = SI_k) 
...
#' @return An object of class 'similar.data'.
#'
#' @details `DataRebuild()` is based on a Gaussian Copula inversion technique also known as NORmal To Anything (NORTA) transformation. If data.rearrange = "norta" is chosen it would mostly make sense to use corrtype = "moment.corr". This program relies on archived package 'JohnsonDistribution', that is one of the few (if not the only) available R code that obtain rather stable solutions for the Johnson system based on the Hill algorithm. Some minimal issues though exist: the SB type distribution (positive reals support) can wrongly sample some negative values. This can be problematic if the marginal is later constrained to be positive (log transformation ?). Then SBjohn.correction = TRUE sets negative values equal to the minimum positive value generated (see function 'rJohn' in source code). This could slightly alter moment features. Hence if no later constraints on the marginal are needed it is better to avoid any correction. Failing of the routine to converge in reasonable time with 'data.rearrange = "norta"' it is likely indication that numerical integration over hypercube has failed. Then 'stochastic.integration = TRUE' should be chosen. 
#' 
#' @section Note: this program currently assumes the input IPD summaries arise from a specific IPD format where a categorical variable with \eqn{m} levels is converted to \eqn{m-1} dummy (binary) variables previous to IPD summary computation. This underlies the simplification made in modelling of the IPD marginals, where the program currently only distinguished between a binary and a non binary type. The latter is approximated by a continuous distribution (either Gamma or Johnson) depending on the amount of marginal moment (respectively either up to second or fourth degree). This particular approach does not need to be restrictive. For instance, in future versions one can adapt the code to allow for categorical marginals as well. For such marginals the preferred modeling choice would intuitively be the Multinomial distribution. 
#'
#'@seealso [Return.key.IPD.summaries()] for allowed input IPD summary format, [FitJohnsonDistribution()] from archived package JohnsonDistribution, [adaptIntegrate()] from package cubature
