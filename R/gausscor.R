...
mccovx1x2 <- function(rz, Gx1, Gx2,..., rx= 0, meanx=c(0,0), sdx=c(1,1), 
...
# rewrite bivariate density without matrix notation
 ...
 c <- 1/(2*pi*sqrt(1-rz^2))

        err <- z[1]^2 - 2*rz*z[1]*z[2] + z[2]^2
        
   point <- c * exp( (-1)*err / (2*(1-rz^2)) )       
...
make.square.matrix <- function( vec, p, from.triangular = T, diag.val = F ){
...
make.matrix.SPD.if.flagged <- function( num.vec, bool.vec, p ){
 ...
 mat <- make.square.matrix( num.vec, p, FALSE)
  ...
  i <- i+1
        ...
        if ( i > 200 ) {  # 50  # increase iteration treshold !!!
...
tweak.to.SPD.sequentially <- function( num.vec, p ){  # num.vec is lower triangular vector here
...
tweak.to.SPD.not.sequentially <- function( num.vec, p ){  # num.vec is lower triangular vector here
...
make.matrix.SPD <- function( mat, flag ){
 ...
 vec <- as.numeric(mat) 
  ...
  if ( any(bool) ) {  # if any matrix element is flagged ....
...
res.bool <- make.square.matrix( unlist(lapply(Rut, function(x) attr(x, "adjusted", T))), p, diag.val = T )  
